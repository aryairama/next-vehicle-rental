      ...
      {props?.types?.map((type, index) => (
        <section key={index} id="populer-in-town" className="container mt-16">
          <div className="flex flex-row flex-wrap w-full justify-between">
            ...
            <div className="flex flex-row w-1/2 justify-end items-center gap-2 cursor-pointer" onClick={()=> router.push(`/type/${type.type_id}`)}>
          ...
          <CardTemplate>
            {type.vehicles.map((vehicle, index) => (
              <CardContainer styleCard="cursor-pointer" onClick={()=> router.push(`/vehicle/${vehicle.vehicle_id}`)} key={index}>
